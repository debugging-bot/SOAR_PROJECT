...
# ---------------------------------------------------------------- 데이터 규격
SEQ_LEN = 30          # 한 시퀀스에 담는 표본 수
FRAME_STRIDE = 2      # 카메라 프레임 몇 장마다 한 장을 담을 것인가
#
#   담기는 시간 = SEQ_LEN x FRAME_STRIDE / 30fps
#               = 30 x 2 / 30 = 2.0초
#   표본 간격   = FRAME_STRIDE / 30fps = 0.067초
#
#   모델 입력은 (30, 146)로 유지되므로 학습 부담이 늘지 않는다.
#   2026-xx-xx 회의에서 1.0초 -> 2.0초로 변경 (촬영 시간이 부족하다는 실측 결과)
...
COOLDOWN_SEC = 2.5      # 확정 직후 이 시간 동안은 새로 확정하지 않음
#   창이 2초이므로 확정 후에도 그 동작이 2초간 버퍼에 남아 있다.
#   쿨다운이 창보다 짧으면 같은 단어가 두 번 확정되므로 창보다 길게 둔다.
PREDICT_EVERY = 2       # 표본 몇 개마다 한 번 예측할 것인가 (프레임이 아니라 표본 기준)
#   실제 예측 주기 = PREDICT_EVERY x FRAME_STRIDE / 30fps = 0.13초 (초당 약 7.5회)
